...
$(function () {
    ...
    $("#form_crop").ajaxForm({
        success: function (n) {
            n.IsSuccess ? ($("#crop_image").attr("src", ""), croper.setImage(""), $("#preview_small").removeAttr("style").attr("src", n.FaceSrc), $("#preview_large").removeAttr("style").attr("src", n.AvatarSrc), $("#crop_operation").hide(), $("#crop_operation_msg").html(""), $("#croped_message").html("裁切并保存成功"), $("#preview_title").html("更新后的头像")) : $("#crop_operation_msg").html('<span style="color:red">操作失败<\/span>')
        },
        error: function () {
            $("#crop_operation_msg").html('<span style="color:red">操作失败<\/span>')
        }
    })
});
var Uploader = function () {
    var n = function (n, t) {
        n.fineUploader({
            ...
            request: {
                endpoint: "/ImageUploader/TemporaryAvatarUpload"
            },
            text: {
